...
const LOCAL_RUNTIME_KEYS = [
  "PETCAM_DEV_USER_EMAIL",
  "PETCAM_BROADCASTER_EMAILS",
  "PETCAM_DEVICE_ID",
  "PETCAM_SHARE_SECRET",
  "KVS_DEVICE_CHANNELS_JSON",
  "KVS_CHANNEL_ARN",
  "KVS_P2P_CHANNEL_ARN",
  "KVS_STORAGE_CHANNEL_ARN",
  "KVS_STREAM_ARN",
  "KVS_BROKER_URL",
  "KVS_BROKER_SECRET",
  "PUSH_BROKER_URL",
  ...
  "DEVICE_PROVISIONING_LEGACY_DEVICE_ID",
];
...
export default defineConfig(async ({ mode }) => {
  ...
  const localRuntimeVariables = Object.fromEntries(
    LOCAL_RUNTIME_KEYS.map((key) => [key, process.env[key] ?? fileEnv[key]]).filter(
